...
客户端 2.x 保留标题栏、左侧导航、中央内容区和房间右侧信息栏的基本分区，视觉系统已统一为克制的深色桌面界面。界面与文档均使用文字和矢量图标表达状态，不使用 emoji。

2026-07-17 完成了 RTMP 发布地址规范化、完整发布地址校验、语音连接状态修复和麦克风断线重连；同时重构公共主题、面板、悬停反馈和窗口标题栏，使页面切换与交互反馈更加一致。
