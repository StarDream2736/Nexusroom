...
| 版本     | 变更内容                                                                                                                                                                                                                                         |
| ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v1.7.0 | **\[屏幕捕获推流]** 新增客户端内置屏幕捕获推流功能：每个推流入口（Ingress）卡片新增"屏幕捕获"按钮，基于内嵌 FFmpeg 子进程实现屏幕采集 → H.264 编码 → RTMP 推流到 SRS；新增 `ScreenCaptureService`（FFmpeg 进程管理，支持全屏/窗口捕获、帧率/码率/NVENC 硬件加速配置）；新增 `ScreenSourceEnumerator`（PowerShell 枚举显示器和窗口列表）；新增 `ScreenCaptureDialog` UI（捕获源选择、画质设置、实时编码统计）；CMake 新增 `tools/ffmpeg.exe` 打包规则；**服务端零改动**——完全复用现有 SRS RTMP→HTTP-FLV 链路 |
...
每个推流入口（Ingress）除了支持 OBS 外部推流外，还内置了屏幕捕获推流功能。客户端捆绑 `ffmpeg.exe`，通过 `dart:io Process` 启动 FFmpeg 子进程进行屏幕采集 + H.264 编码 + RTMP 推流，完全复用现有的 SRS RTMP→HTTP-FLV 分发链路，**服务端零改动**。

```
用户点击 Ingress 卡片 "屏幕捕获" 按钮
        ↓
ScreenCaptureDialog：选择采集源 + 画质配置
        ↓
ScreenCaptureService.startCapture()
        ↓
ffmpeg.exe -f gdigrab -i desktop -c:v libx264 -f flv rtmp://IP:1935/live/{stream_key}
        ↓
...
| ScreenCaptureService | `core/native/screen_capture_service.dart` | FFmpeg 进程生命周期管理、stderr 解析编码统计 |
| ScreenSourceEnumerator | `core/native/screen_source_enumerator.dart` | 通过 PowerShell 枚举可用显示器和窗口 |
| ScreenCaptureDialog | `features/room/presentation/widgets/screen_capture_dialog.dart` | UI：采集源选择、画质配置、实时编码统计 |
| screen_capture_provider | `features/room/presentation/providers/screen_capture_provider.dart` | Riverpod 状态管理（CaptureStatus / CaptureStats） |

### 7.5.3 支持的捕获模式

| 模式 | FFmpeg 参数 | 说明 |
|------|------------|------|
| 全屏（主显示器） | `-f gdigrab -i desktop` | 捕获整个桌面 |
| 全屏（副显示器） | `-f gdigrab -offset_x X -offset_y Y -video_size WxH -i desktop` | 通过偏移和尺寸选定特定显示器 |
| 窗口模式 | `-f gdigrab -i title=窗口标题` | 捕获指定标题的窗口 |

### 7.5.4 编码配置

| 参数 | 默认值 | 可选值 | 说明 |
|------|-------|--------|------|
| 帧率 (FPS) | 30 | 15 / 24 / 30 / 60 | 值越高越流畅，CPU 占用越大 |
| 码率 (Bitrate) | 3000 kbps | 1500 / 3000 / 5000 / 8000 | 值越高画质越好，带宽要求越高 |
| 编码器 | libx264 (软件) | h264_nvenc (NVIDIA 硬件加速) | NVENC 大幅降低 CPU 占用 |
| 预设 (Preset) | veryfast | — | 配合 `-tune zerolatency` 低延迟优化 |
...
| ffmpeg.exe（内嵌二进制） | 屏幕捕获推流引擎（gdigrab 采集 → H.264 编码 → RTMP 推流到 SRS），通过 dart:io Process 管理 |
