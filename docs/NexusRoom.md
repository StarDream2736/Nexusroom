...
- `docs/development/` 是被 Git 忽略的本地工作日志，不得被公开文档链接依赖。
- `docx/` 是历史日志归档，内容可能过时，不得覆盖当前源码事实。
...
1. `%LOCALAPPDATA%\NexusRoom\data\nexusroom.sqlite`。
2. 系统“文档”目录中的 `nexusroom.sqlite`。
3. `Nexusroom.exe` 同级的 `data\nexusroom.sqlite`。

迁移必须包含主文件、WAL 和 SHM，使用临时文件完成后再重命名。目标数据库已经存在时不得覆盖。
...
重构可以改变组件表面、排版、间距、动画、控件和反馈，但不得在没有产品设计决策的情况下破坏四区信息架构。
...
- UI 不使用 emoji；图标使用 Material 或项目矢量资源。
...
- 不提交生成物、运行数据、密钥、开发日志或本机二进制依赖。
- 源码、公开文档和 UI 不使用 emoji；必要状态使用文本和矢量图标。
...
- 客户端迁移不得覆盖已有目标数据库，不得丢失 WAL 中未检查点数据。
- 所有账号级本地查询必须包含账号隔离键。
...
- 正确实现不会关闭全局数据库，也不会删除 data 目录。
- 清理后 `data\nexusroom.sqlite` 应仍存在并可重新写入服务器设置。
- 若使用旧版客户端删除了目录，重新启动会创建空数据库；需要从备份恢复原数据。
...
- [ ] CHANGELOG 是否按语义版本记录。
- [ ] 文档本地链接是否有效。
- [ ] 公开文档和 UI 是否不存在 emoji。
- [ ] Obsidian Vault 中的镜像是否从仓库规范重新同步。

## 22. 版本变更摘要

### 2.1.0 - 2026-07-17

- 保留四区布局，重构完整桌面 UI 和黑白明暗主题。
- 通过 ThemeExtension 让所有已挂载组件同步切换主题。
- 客户端持久化数据迁移到 `Nexusroom.exe` 同级 data 目录。
- 消息缓存按服务器和账号隔离。
- 修复清除数据删除目录和关闭数据库导致进程不可继续运行的问题。
- 语音检测增加双阈值与静音保持，成员中心点常亮、外围光晕呼吸。
- 修复麦克风连接、房间消息与图片发送、推流地址和关闭生命周期问题。
- 完善开源许可证、贡献、安全、行为准则、发布历史和集中技术文档。

### 2.0.0 - 2026-07-15

- 服务端从多服务编排重构为单一 Go 进程。
- 使用 SQLite 替代 PostgreSQL，移除 Redis。
- 使用内建 Opus SFU、RTMP、HTTP-FLV、WebRTC 播放和 TURN。
- 网页资源编译进服务端二进制。
- Docker 收敛为单容器，并保留 Linux 直接部署。
