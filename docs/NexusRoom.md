...
- 服务端账号注册、登录、资料和好友关系 API；当前桌面 UI 在紧凑认证窗口提供登录、注册和会话恢复。
- 房间创建、加入、成员管理和在线状态。
...
这张表是服务端完整 API 清单。当前 Electron UI 调用注册、登录、房间、消息、图片、直播入口和 VLAN 相关路径；好友、任意文件、资料、管理员和 Webhook 仍是服务端或集成者接口。桌面注册流程不暴露 `admin_token` 输入。
...
当前 Electron UI 的实现范围是：未登录时在紧凑认证窗口填写服务器地址并登录或注册，认证成功后同一 BrowserWindow 展开完整 UI 并恢复会话；查看房间列表、创建、邀请码加入、退出和切换；加载历史与实时文字；上传图片并使用会话鉴权的 Blob 显示；加入 WebRTC 语音、静音并显示成员在线和说话状态；管理直播入口并优先 WebRTC 播放，失败后锁定 HTTP-FLV 直到刷新；启用 WireGuard VLAN。桌面注册流程不暴露 `admin_token` 输入。服务端虽然有好友、任意文件和管理 API，桌面 UI 当前没有这些页面，也没有屏幕采集、音频设备选择或客户端 FFmpeg 推流功能。
...
未登录时窗口固定为紧凑认证状态，只显示认证表单；登录、注册或恢复有效会话后，同一窗口展开上述分区，不刷新页面，也不创建第二个窗口。

界面组件、排版、间距、动画和交互状态由 React 组件和共享 CSS token 实现。当前 UI 只声明登录、注册、房间、消息、语音、直播和 VLAN 的已实现状态。
