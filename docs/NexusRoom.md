...
| v1.7.6 | **\[窗口捕获重写]** 放弃 gdigrab `title=` 模式（BitBlt 方式对 DirectX/硬件加速窗口黑屏），改为 desktop+offset+size 裁剪方式（通过 DWM 合成捕获）；**\[系统音频]** 恢复系统音频设备选择器；**\[音频同步]** 麦克风输入添加 `-use_wallclock_as_timestamps 1` + `aresample=async=1000` 修复时钟漂移 |
| v1.7.5 | **\[-vf 位置修复]** 将 `-vf` 从视频输入和音频输入之间移至所有输入之后（FFmpeg 误将其解析为音频输入选项导致致命错误）；**\[窗口标题编码]** 修复含 ® 等特殊字符的窗口标题因 codepage 转换损坏（改用 `Process.start` + UTF-8 解码 + `[Console]::OutputEncoding=UTF8`） |
| v1.7.4 | **\[UI 溢出修复]** Dialog 内容区包裹 `Flexible` + `SingleChildScrollView` 解决内容超出屏幕不可滚动的问题；**\[CJK 音频设备名]** 音频设备枚举改用 `Process.start` 捕获原始字节 + UTF-8 解码，解决中文 Windows GBK codepage 导致的设备名乱码；**\[系统音频选择器]** 移除冗余的系统音频设备选择器（当时认为不需要）；**\[按钮字体]** "关闭" 按钮改用 `TextButton.styleFrom(foregroundColor: ...)` 保持字体一致性 |
...
系统采用三层分离架构：前端客户端（Flutter Desktop + Browser Web Page）、后端主服务（Golang）、音视频引擎（LiveKit + SRS）。桌面客户端通过 WebSocket 连接服务端，语音通话通过 LiveKit WebRTC 通道传输；OBS 推流通过 SRS（RTMP 接入 → HTTP-FLV 分发）实现。网页直播页由 Go 主服务直接托管，采用 WebRTC 优先 + FLV 回退并提供前端诊断日志。
...
│   │   ├── handler/           # 各业务 Handler
│   │   │   └── web_public.go  # 网页直播公开接口（房间列表、RTC 播放代理）
...
├── web/                       # 网页直播静态页面
│   ├── index.html             # 直播房间列表页
│   ├── player.html            # WebRTC/FLV 播放页
│   ├── srs.sdk.js             # SRS WebRTC SDK
│   └── mpegts.min.js          # FLV 播放依赖（本地优先）
└── config.yaml                # 服务端配置文件
```
...
│   │   ├── native/            # Platform Channel（wireguard_service.dart、screen_capture_service.dart、screen_source_enumerator.dart）
...
### 5.10 网页直播公开 API（无需认证）

用于 Browser 访问 `index.html` / `player.html` 的公开接口。

| 方法 | 路径 | 权限 | 说明 |
|------|------|------|------|
| GET | /web/rooms/live | 公开 | 聚合 SRS 当前活跃流并映射到房间列表 |
| POST | /web/rtc/play | 公开 | WebRTC 播放信令代理（转发到 SRS `/rtc/v1/play/`） |
| GET | /stream/:streamKey | 公开 | HTTP-FLV 反向代理（FLV 回退播放） |

#### POST /web/rtc/play（代理策略）

为避免整合部署场景下 SRS 对播放上下文识别偏差（常见现象：`connected` 但 `inbound-rtp bytes=0`），服务端在转发前会对请求体做标准化处理：

- 强制 `api` 指向 SRS 实际入口：`http://srs:1985/rtc/v1/play/`
- 注入 `clientip`
- 规范 `streamurl`，当查询参数缺失时自动补全 `vhost=__defaultVhost__`

该策略用于统一“浏览器 -> 主服务代理 -> SRS”三段上下文，降低 WebRTC 空媒体问题。

#### GET /web/rooms/live（响应示例）
    ...
    "rooms": [
      {
        ...
        "streams": [
          {
            "app": "live",
            "stream_key": "bf868b9a-90b",
            "label": "OBS主推流",
            "clients": 0,
            "bitrate_kbps": 3178,
            "play_url": "/api/v1/stream/bf868b9a-90b"
          }
        ]
      }
    ]
...
### 9.6.1 网页播放策略（WebRTC 优先）

网页播放页采用“WebRTC 优先、FLV 回退”的双通道策略，并内置自动诊断：

1. 优先使用 WebRTC **音视频协商（AV）**。
2. 若 WebRTC `connected` 且一段时间内无入站媒体（`inbound-rtp bytes=0`），自动降级为 `videoOnly` 再试。
3. 若仍无首帧或无可用媒体，回退到 FLV。
4. 若出现“视频有流量但音频无流量（audio bytes=0）”，回退 FLV 以保证有声播放。

诊断判据：

- `bytes=0`：通常是源流兼容或播放上下文不匹配问题（非单纯 ICE）。
- `video>0 audio=0`：WebRTC 音频链路不兼容（常见于 HE-AAC 等）。
- `video>0` 且 `readyState<2`：有媒体但无首帧，多与关键帧间隔/解码时机相关。
...
## 7.5 客户端内置屏幕捕获推流（FFmpeg 方案）

### 7.5.1 架构说明

每个推流入口（Ingress）除了支持 OBS 外部推流外，还内置了屏幕捕获推流功能。客户端捆绑 `ffmpeg.exe`，通过 `dart:io Process` 启动 FFmpeg 子进程进行屏幕采集 → H.264 CBR 编码 → RTMP 推流到 SRS，完全复用现有的 SRS RTMP→HTTP-FLV 分发链路，**服务端零改动**。仅支持全屏（显示器）捕获，纯视频推流（无音频）。

```
用户点击 Ingress 卡片 "屏幕捕获" 按钮
        ↓
ScreenCaptureDialog：选择显示器 + 画质配置
        ↓
ScreenCaptureService.startCapture()
        ↓
ffmpeg.exe -f gdigrab -i desktop -c:v h264_nvenc -rc cbr -vf ... -an -f flv rtmp://IP:1935/live/{key}
        ↓
SRS on_publish 回调 → room_ingresses.is_active = true → WS 广播 stream.new
        ↓
其他客户端 media_kit 播放 HTTP-FLV（现有流程不变）
```
...
| 组件 | 位置 | 作用 |
|------|------|------|
| ffmpeg.exe | 客户端安装包（`tools/` 目录打包） | 屏幕采集 + 视频编码 + RTMP 推流 |
| ScreenCaptureService | `core/native/screen_capture_service.dart` | FFmpeg 进程生命周期管理（启动/停止/dispose）、stderr 解析编码统计（FPS/码率/时长/帧数） |
| ScreenSourceEnumerator | `core/native/screen_source_enumerator.dart` | 通过 PowerShell 枚举可用显示器（`System.Windows.Forms.Screen`） |
| ScreenCaptureDialog | `features/room/presentation/widgets/screen_capture_dialog.dart` | UI：显示器选择、画质配置（帧率/码率/硬件加速）、实时编码统计 |
| screen_capture_provider | `features/room/presentation/providers/screen_capture_provider.dart` | Riverpod 状态管理（CaptureStatus / CaptureStats / DisplaySources） |

### 7.5.3 支持的捕获模式

| 模式 | FFmpeg 参数 | 说明 |
|------|------------|------|
| 全屏（主显示器） | `-f gdigrab -i desktop` | 捕获整个主桌面 |
| 全屏（副显示器） | `-f gdigrab -offset_x X -offset_y Y -video_size WxH -i desktop` | 通过偏移和尺寸选定特定显示器 |

> **已移除**：窗口捕获模式（v1.7.1–v1.7.7 期间曾支持，经历了 `gdigrab title=` → `desktop+offset+crop` 两种方案，因复杂度和稳定性问题在 v1.7.9 中移除）和音频捕获（系统音频/麦克风，因 FFmpeg dshow 时钟同步导致视频严重卡顿，在 v1.7.9 中移除）。

### 7.5.4 编码配置

| 参数 | 默认值 | 可选值 | 说明 |
|------|-------|--------|------|
| 帧率 (FPS) | 60 | 15 / 24 / 30 / 60 / 120 | 值越高越流畅，CPU/GPU 占用越大 |
| 码率 (Bitrate) | 8000 kbps (推荐) | 1500(流畅) / 3000(标清) / 5000(高清) / 8000(推荐) / 10000(超清) / 12000(极清) | CBR 恒定码率，值越高画质越好 |
| 编码器 | h264_nvenc (NVIDIA 硬件加速，默认开启) | libx264 (软件编码) | NVENC 大幅降低 CPU 占用，推荐使用 |
| 码率控制 | CBR | — | NVENC: `-rc cbr`；x264: `-b:v = -maxrate = -minrate`，确保实际码率与设定值一致 |
| 预设 (Preset) | NVENC: p4 / x264: veryfast | — | x264 配合 `-tune zerolatency` 低延迟优化 |
| 关键帧间隔 | fps × 2 | — | 保证直播流的快速 seek 和切换 |
| 音频 | 禁用 (`-an`) | — | 纯视频推流，无音频 |
...
- `fps={fps}`：将 gdigrab 的不规则 VFR 输出重采样为精确 CFR
- `pad=ceil(iw/2)*2:ceil(ih/2)*2`：将奇数分辨率填充为偶数（libx264 要求）

### 7.5.6 已解决的关键问题

| 问题 | 根因 | 解决方案 | 版本 |
|------|------|----------|------|
| 直播画面卡顿 | gdigrab 实时源缺少缓冲区 | 添加 `-rtbufsize 150M` + `-thread_queue_size 1024` | v1.7.1 |
| 奇数分辨率编码崩溃 | libx264 要求偶数尺寸 | 视频滤镜添加 `pad=ceil(iw/2)*2:ceil(ih/2)*2` | v1.7.1 |
| 窗口捕获黑屏 | gdigrab `title=` 使用 BitBlt，DirectX 窗口不兼容 | 改用 desktop+offset+crop（DWM 合成） | v1.7.6 |
| 窗口尺寸解析颠倒 | PowerShell 输出的 width/height 变量赋值反了 | 修正解析顺序 | v1.7.7 |
| CJK 设备名乱码 | `Process.run` 使用系统 codepage (GBK) | 改用 `Process.start` + 原始字节 UTF-8 解码 | v1.7.4 |
...
FFmpeg 通过 CMake `install()` 在构建时自动捆绑到应用目录：
...
srs:
  ...
  api_port: 1985                  # SRS HTTP API / RTC API 内部端口
...
|------|------|------|
| 8080 | TCP | 主服务 API / WS |
| 8881 | TCP | 网页直播入口（index/player） |
| 1935 | TCP | SRS RTMP 推流 |
| 8883 | TCP | SRS RTMP 别名 |
| 8000 | UDP | SRS WebRTC 媒体 |
| 51820 | UDP | WireGuard VLAN |
| 7880 | TCP | LiveKit 信令 |
| 7881 | TCP | LiveKit TCP 穿透 |
| 3478 | UDP | TURN |
...
| window_manager | Flutter Desktop 窗口状态监听（最小化/失焦检测，后台挂起视频解码；窗口关闭时自动终止 FFmpeg 进程） |
| ffmpeg.exe（内嵌二进制） | 屏幕捕获推流引擎（gdigrab 采集 → H.264 CBR 编码 → RTMP 推流到 SRS），通过 dart:io Process 管理，仅视频无音频 |
...
*NexusRoom Technical Documentation v1.8.2*
